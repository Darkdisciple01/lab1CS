...
def add_user(username, password, key, init_vec, filename='data.json'):
    ...
    new_data = {"username": str(username),
            "password": (password_b64), "key": str(key_b64), "init_vec": str(init_vec_b64)}
...
def load_file_data():
    ...
    for account in data["users"]:
        ...
        key_database.append(b64decode(account["key"].encode('utf-8')))
        init_vec_database.append(b64decode(account["init_vec"].encode('utf-8')))

    return user_database, pass_database, key_database, init_vec_database
